...
const tabs = ['Squad', 'Matches', 'Overview', 'Attack', 'Defense']
const activeTab = ref(0)

watch(activeTab, (tab) => {
  if (tab === 2 && !overview.value) fetchOverview()
  if (tab === 3 && !attack.value) fetchAttack()
  if (tab === 4 && !defense.value) fetchDefense()
})
...
<template>
  <div class="space-y-6">
    ...
    <template v-else-if="team">
      ...
      <UTabs :items="tabs.map(t => ({ label: t }))" v-model="activeTab" />
